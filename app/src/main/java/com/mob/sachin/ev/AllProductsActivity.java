...
public class AllProductsActivity extends ListActivity {
    ...
    public void onCreate(Bundle savedInstanceState) {
        ...
        Button btnProceed = (Button) findViewById(R.id.proceedButton);
        btnProceed.setOnClickListener(new View.OnClickListener() {

            @Override
            public void onClick(View view) {
                ArrayList<ProductDetails> confirmProductList = new ArrayList<>();
                // Launching All products Activity
                Intent i = new Intent(AllProductsActivity.this, PaymentActivity.class);

                //i.putExtra("selectedProductList", productDetailsArrayList);
                for (int index = 0; index < productDetailsArrayList.size(); index++) {
                    ProductDetails product = productDetailsArrayList.get(index);
                    int val = Integer.parseInt(product.selectedQuantity);
                    if(val != 0){
                        confirmProductList.add(product);
                    }
                }
                Bundle productDisplayForConfirmation = new Bundle();
                productDisplayForConfirmation.putSerializable("confirmProductList", confirmProductList);
                i.putExtras(productDisplayForConfirmation);
                startActivity(i);
